...
async def get_subscription_status(
    ...
    async with AsyncTraceContext("api.get_subscription_status"):
        ...
        logger.info(
            ...
            extra={"user_id": str(user_id), "is_pro": is_active}
        )
...
async def start_trial(
    ...
    """
    logger.info(
        "Trial start requested",
        extra={"user_id": str(user_id), "trial_days": trial_days}
    )
    ...
    async with AsyncTraceContext("api.start_trial"):
        ...
        if existing_sub:
            raise HTTPException(
                status_code=400,
                detail="User already has a subscription or trial"
            )
