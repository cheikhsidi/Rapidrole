...
from utils.logging import get_logger, log_agent_execution
        ...
        async with AsyncTraceContext("workflow.analyze_job"):
            ...
            logger.info(
                ...
                extra={
                    ...
                    "required_skills_count": len(state["compatibility_analysis"]["required_skills"]),
                }
            )
...
async def optimize_resume_node(state: ApplicationState) -> ApplicationState:
    ...
    logger.info(
        "Starting resume optimization workflow node",
...
async def generate_cover_letter_node(state: ApplicationState) -> ApplicationState:
    ...
    logger.info(
        "Starting cover letter generation workflow node",
        extra={"user_id": state.get("user_id")}
    )
