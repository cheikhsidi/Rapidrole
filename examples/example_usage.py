...
async def main():
    async with httpx.AsyncClient() as client:
        ...
        user_response = await client.post(
            f"{BASE_URL}/users/", json={"email": "john.doe@example.com", "full_name": "John Doe"}
        )
        ...
        print("✅ Profile created with embeddings")

        # 3. Search for compatible jobs
        print("\n🔍 Searching for compatible jobs...")
        jobs_response = await client.get(
            f"{BASE_URL}/jobs/search", params={"user_id": user_id, "limit": 5, "min_score": 0.7}
        )
        ...
        if jobs_data["jobs"]:
            ...
            app_response = await client.post(
                f"{BASE_URL}/applications/", json={"user_id": user_id, "job_id": job_id}
            )
            ...
            rec_response = await client.get(f"{BASE_URL}/intelligence/recommendations/{user_id}")
